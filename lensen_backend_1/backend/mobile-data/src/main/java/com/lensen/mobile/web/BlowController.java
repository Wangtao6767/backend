...
public class BlowController {
     ...
     * 获取鸣笛记录列表（H5端）
     */
    @PostMapping(value = "/list_h5", produces = "application/json;charset=UTF-8")
    public String list1(@RequestBody(required = false) BlowDto dto) {
        long start = System.currentTimeMillis();
        BaseResponse<Object> result = blowAO.selectBlowList1(dto);
        LOGGER.info("blow/list cost: {}ms", System.currentTimeMillis() - start);
        return CommonUtil.responseFromObject(result);
    }
